{{-- @if (Auth::user()->hasPermissionTo('bbm-penerimaan') || Auth::user()->hasPermissionTo('bbm-pemakaian')) --}}
    @php
       ...
       if(
            request()->routeIs('inputs.biomassa_receipts.index') ||
            request()->routeIs('biomassa.usages.index')
            ){
                $isOpen = true;
            }
   @endphp
        ...
        <div x-data="{open:{{ $isOpen ? $isOpen : 'false' }}}">
            ...
            <div x-cloak x-show="open" x-transition:enter="transition-transform transition-opacity ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-[-10%]" x-transition:enter-end="opacity-100 translate-y-0" class="px-5 py-3 text-[#ffffff]">
                {{-- @if (Auth::user()->hasPermissionTo('bbm-pemakaian')) --}}
                    <div>
                        <a href="{{ route('inputs.biomassa_receipts.index') }}">
                    ...
                    <div>
                        <a href="{{ route('biomassa.usages.index') }}">
                            <div class="flex items-center gap-4 py-2 cursor-pointer hover:scale-105 duration-300 hover:bg-[#047A96] {{ request()->routeIs('biomassa.usages.index') ? 'bg-[#047A96]' : 'hover:bg-[#047A96]' }} rounded-lg">
                                <div>
                                    <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="12" cy="12.0005" r="5" fill="#ffffff"/>
                                    </svg>
