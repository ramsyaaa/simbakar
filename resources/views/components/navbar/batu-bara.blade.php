@if (Auth::user()->hasPermissionTo('batu-bara-pembongkaran') || Auth::user()->hasPermissionTo('batu-bara-penerimaan') || Auth::user()->hasPermissionTo('batu-bara-pemakaian'))
    @php
       $isOpen = false;

       if(
            request()->routeIs('coals.unloadings.index') ||
            request()->routeIs('coals.receipts.index') ||
            request()->routeIs('coals.usages.index')
            ){
                $isOpen = true;
            }
   @endphp
    <div>
        <div x-data="{open:{{ $isOpen ? $isOpen : 'false' }}}">
            ...
            <div x-cloak x-show="open" x-transition:enter="transition-transform transition-opacity ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-[-10%]" x-transition:enter-end="opacity-100 translate-y-0" class="px-5 py-3 text-[#ffffff]">
                @if (Auth::user()->hasPermissionTo('batu-bara-pembongkaran'))
                    <div>
                        <a href="{{route('coals.unloadings.index')}}">
                            <div class="flex items-center gap-4 py-2 cursor-pointer hover:scale-105 duration-300 hover:bg-[#047A96] {{ request()->routeIs('coals.unloadings.index') ? 'bg-[#047A96]' : 'hover:bg-[#047A96]' }} rounded-lg">
                                <div>
                                    <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                @endif
                @if (Auth::user()->hasPermissionTo('batu-bara-penerimaan'))
                    <div>
                        <a href="{{route('coals.receipts.index')}}">
                            <div class="flex items-center gap-4 py-2 cursor-pointer hover:scale-105 duration-300 hover:bg-[#047A96] {{ request()->routeIs('coals.receipts.index') ? 'bg-[#047A96]' : 'hover:bg-[#047A96]' }} rounded-lg">
                                <div>
                                    <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="12" cy="12.0005" r="5" fill="#ffffff"/>
                                    </svg>
                ...
                @endif
                @if (Auth::user()->hasPermissionTo('batu-bara-pemakaian'))
                    <div>
                        <a href="{{route('coals.usages.index')}}">
                            <div class="flex items-center gap-4 py-2 cursor-pointer hover:scale-105 duration-300 hover:bg-[#047A96] {{ request()->routeIs('coals.usages.index') ? 'bg-[#047A96]' : 'hover:bg-[#047A96]' }} rounded-lg">
                                <div>
                                    <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <circle cx="12" cy="12.0005" r="5" fill="#ffffff"/>
                                    </svg>
