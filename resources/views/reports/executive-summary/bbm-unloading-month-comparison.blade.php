...
@section('content')
                            ...
                            <tbody>
                                ...
                                @foreach ($bbm_unloading as $month => $item)
                                    ...
                                    <tr>
                                        ...
                                        <td class="h-[36px] text-[16px] font-normal border px-2">{{ $item['bl'] ?? '-' }}
                                        </td>
                                        <td class="h-[36px] text-[16px] font-normal border px-2">{{ $item['ds'] ?? '-' }}
                                        </td>
                                        <td class="h-[36px] text-[16px] font-normal border px-2">{{ $item['bw'] ?? '-' }}
                                        </td>
                                        <td class="h-[36px] text-[16px] font-normal border px-2">{{ $item['tug'] ?? '-' }}
                                        </td>
                                        <td class="h-[36px] text-[16px] font-normal border px-2">{{ $item['ds_bl'] ?? '-' }}
                                        </td>
                                        <td class="h-[36px] text-[16px] font-normal border px-2">
                                            {{ $item['ds_bl_percentage'] ?? '-' }}</td>
                                        <td class="h-[36px] text-[16px] font-normal border px-2">{{ $item['bw_ds'] ?? '-' }}
                                        </td>
                                        <td class="h-[36px] text-[16px] font-normal border px-2">
                                            {{ $item['bw_ds_percentage'] ?? '-' }}</td>
                                        <td class="h-[36px] text-[16px] font-normal border px-2">
                                            {{ $item['bl_bw'] ?? '-' }}</td>
                                        <td class="h-[36px] text-[16px] font-normal border px-2">
                                            {{ $item['bl_bw_percentage'] ?? '-' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
    ...
    <script>
        ...
        bl = bl.map((item) => item ? parseInt(item.replaceAll(',', '')) : 0)
        ds = ds.map((item) => item ? parseInt(item.replaceAll(',', '')) : 0)
        bw = bw.map((item) => item ? parseInt(item.replaceAll(',', '')) : 0)
