    ...
    <div :class="sidebar?'w-10/12' : 'w-full'">
        ...
        <div class="w-full py-10 px-8">
            <div class="flex items-end justify-between mb-2">
                <div>
                    <div class="text-[#135F9C] text-[40px] font-bold">
                        Tambah Adendum Klausul Penyesuaian Kontrak Batu Bara
                    </div>
                    <div class="mb-4 text-[16px] text-[#6C757D] font-normal no-select">
                        <a href="{{ route('administration.dashboard') }}">Home</a> <a href="{{ route('contracts.adendum-coal-contracts.index',['contractId' => $adendum->contract_id]) }}" class="cursor-pointer">  <a href="{{ route('contracts.adendum-coal-contracts.index',['contractId' => $adendum->contract_id]) }}"> / Adendum Kontrak Batu Bara  <a href="{{ route('contracts.adendum-coal-contracts.adjusment-clause.index',['adendumId' => $adendum->id]) }}"> / Klausul Penyesuaian Kontrak Batu Bara / </a> <span class="text-[#2E46BA] cursor-pointer">{{$contract->contract_number}}</span>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-lg p-6">
                ...
                <form onsubmit="return confirmSubmit(this, 'Tambahkan Klausul Penyesuaian Kontrak Baru ?')" action="{{ route('contracts.adendum-coal-contracts.adjusment-clause.store',['adendumId'=>$adendum->id]) }}" method="POST">
                    @csrf
                    ...
                    <small class="text-slate-700"> Untuk angka koma gunakan titik .</small>
                        ...
                        <div class="w-full py-1 flex gap-3">
                            <input type="hidden" value="{{$adendum->id}}" name="contract_id">
                            <div class="delivery">
                        ...
                        <div class="flex gap-3">
                            <a href="{{route('contracts.adendum-coal-contracts.adjusment-clause.index',['adendumId'=>$adendum->id])}}" class="bg-[#C03221] w-full h-full lg:w-[300px] py-3 text-[white] text-[16px] font-semibold rounded-lg mt-3 px-3 text-center">Back</a>
