    ...
    <div :class="sidebar?'w-10/12' : 'w-full'">
        ...
        <div class="w-full py-10 px-8">
            ...
            <div class="bg-white rounded-lg p-6">
                ...
                <form onsubmit="return confirmSubmit(this, 'Ubah Sub Supplier Kontrak Baru ?')" action="{{ route('contracts.biomassa-contracts.sub-supplier.update',['contractId'=>$contract->id,'id'=>$sub->id]) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="w-full py-1">
                        ...
                        <div class="relative">
                            <select name="supplier_id" id="" class="select-2 w-full lg:w-[600px] h-[44px] text-[19px] text-[#8A92A6] border rounded-md">
                                ...
                                <option value="{{ $supplier->id }}" {{$sub->supplier_id == $supplier->id ? 'selected' : ''}}>{{ $supplier->name }}</option>
                                @endforeach
                            </select>
                            @error('supplier_id')
