            ...
            <div class="flex items-end justify-between mb-2">
                <div>
                    <div class="text-[#135F9C] text-[40px] font-bold">
                        List Kontrak Biomassa
                    </div>
                    <div class="mb-4 text-[16px] text-[#6C757D] font-normal no-select">
                        <a href="{{ route('administration.dashboard') }}">Home</a> / <span class="text-[#2E46BA] cursor-pointer">Kontrak Biomassa</span>
                    </div>
                </div>
                <div class="flex gap-2 items-center">
                    <a href="{{ route('contracts.biomassa-contracts.create') }}" class="w-fit px-2 lg:px-0 lg:w-[200px] py-1 lg:py-2 text-white bg-[#222569] rounded-md text-[12px] lg:text-[19px] text-center">
                        Tambah Data
                    </a>
                </div>
            </div>
            <div class="bg-white rounded-lg p-6">
                <form x-data="{ submitForm: function() { document.getElementById('filterForm').submit(); } }" x-on:change="submitForm()" action="{{ route('contracts.biomassa-contracts.index') }}" method="GET" id="filterForm">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr>
                                ...
                                <th class="border text-white bg-[#047A96] h-[52px]">Nomor Kontrak</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="h-[36px] text-[16px] font-normal border px-2 text-center">
                                    <a href="{{ route('contracts.biomassa-contracts.edit',['uuid'=>$biomassa->uuid]) }}" class="text-sky-700">
                                        {{ $biomassa->contract_number }}
                                    </a>
                                    <br/>
                                    <span>Jenis : {{$biomassa->kind_contract}}</span>
                                    <br/>
                                    [Ada Adendum]
                                    <br/>
                                    <span>Jenis Kontrak Baru : {{$biomassa->kind_contract}}</span>
                                </td>
                                <td class="h-[36px] text-[16px] font-normal border px-2 text-center">
                                    <ul class="text-center">
                                        <li>
                                            <a href="{{route('contracts.biomassa-contracts.spesification.index',['contractId' => $biomassa->id])}}" class="text-sky-700 hover:text-sky-900">
                                                Spesifikasi Biomassa ( {{$biomassa->spesifications->count()}} )
                                            </a>
                                        </li>
                                        <li>
...
{{--
                                        ...
                                        <form onsubmit="return confirmSubmit(this, 'Hapus Data?')" action="{{ route('contracts.biomassa-contracts.destroy', ['uuid' => $biomassa->uuid]) }}" method="POST">
                                            @csrf
                                            @method('delete')
