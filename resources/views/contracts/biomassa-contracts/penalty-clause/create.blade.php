    ...
    <div :class="sidebar?'w-10/12' : 'w-full'">
        ...
        <div class="w-full py-10 px-8">
            <div class="flex items-end justify-between mb-2">
                <div>
                    <div class="text-[#135F9C] text-[40px] font-bold">
                        Tambah Denda Penolakan Kontrak Biomassa
                    </div>
                    <div class="mb-4 text-[16px] text-[#6C757D] font-normal no-select">
                        <a href="{{ route('administration.dashboard') }}">Home</a> <a href="{{ route('contracts.biomassa-contracts.index') }}" class="cursor-pointer">  <a href="{{ route('contracts.biomassa-contracts.index') }}"> / Kontrak Biomassa  <a href="{{ route('contracts.biomassa-contracts.penalty-clause.index',['contractId' => $contract->id]) }}"> / Denda Penolakan Kontrak Biomassa / </a> <span class="text-[#2E46BA] cursor-pointer">{{$contract->contract_number}}</span>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-lg p-6">
                ...
                <form onsubmit="return confirmSubmit(this, 'Tambahkan Denda Penolakan Kontrak Baru ?')" action="{{ route('contracts.biomassa-contracts.penalty-clause.store',['contractId'=>$contract->id]) }}" method="POST">
                    @csrf
                    ...
                    <small class="text-slate-700"> Untuk angka koma gunakan titik .</small>
                        ...
                        <div class="flex gap-3">
                            <a href="{{route('contracts.biomassa-contracts.penalty-clause.index',['contractId'=>$contract->id])}}" class="bg-[#C03221] w-full h-full lg:w-[300px] py-3 text-[white] text-[16px] font-semibold rounded-lg mt-3 px-3 text-center">Back</a>
