            ...
            <div class="flex items-end justify-between mb-2">
                <div>
                    ...
                    <div class="mb-4 text-[16px] text-[#6C757D] font-normal no-select">
                        <a href="{{ route('administration.dashboard') }}">Home</a> / <span class="cursor-pointer">TUG 4 </span>
                    </div>
                </div>      
            </div>
            
            <div class="bg-white rounded-lg p-6 h-full">
                <form x-data="{ submitForm: function() { document.getElementById('filterForm').submit(); } }" x-on:change="submitForm()" action="{{ route('inputs.tug-4.index') }}" method="GET" id="filterForm">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr>
                                ...
                                <th class="border  bg-[#F5F6FA] h-[52px] text-[#8A92A6]">No BPB</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="h-[36px] text-[16px] font-normal border px-2 text-center">{{ $tug->created_at->format('Y-m-d') }}</td>
                                <td class="h-[36px] text-[16px] font-normal border px-2 text-center">{{ $tug->bpb_number }}</td>
                                ...
                                <td class="h-[108px] text-[16px] font-normal border px-2 flex items-center justify-center gap-2">
                                    ...
                                    <a href="{{route('inputs.tug-4.edit',['id' => $tug->id])}}" class="bg-[#1AA053] text-center text-white w-[80px] h-[25px] text-[16px] rounded-md">
                                        Edit
                                    </a>
                                    <form onsubmit="return confirmSubmit(this, 'Hapus Data?')" action="{{ route('inputs.tug-4.destroy', ['id' => $tug->id]) }}" method="POST">
                                        @csrf
                                        @method('delete')
