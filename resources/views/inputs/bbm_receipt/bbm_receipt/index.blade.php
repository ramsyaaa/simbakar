        ...
        <div class="w-full py-10 px-8">
            <div class="flex items-end justify-between mb-2">
                ...
                <div class="flex gap-2 items-center">
                    <a href="{{ route('inputs.bbm_receipts.create') }}" class="w-fit px-2 lg:px-0 lg:w-[200px] py-1 lg:py-2 text-white bg-[#222569] rounded-md text-[12px] lg:text-[19px] text-center">
                        Tambah Data
                    </a>
                </div>
            </div>
            <div class="bg-white rounded-lg p-6">
                <form x-data="{ submitForm: function() { document.getElementById('filterForm').submit(); } }" x-on:change="submitForm()" action="{{ route('inputs.bbm_receipts.index') }}" method="GET" id="filterForm">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="h-[36px] text-[16px] font-normal border px-2 flex items-center justify-center gap-2">
                                    <a href="{{ route('inputs.bbm_receipts.edit', ['id' => $item->id]) }}" class="bg-[#1AA053] text-center text-white w-[80px] h-[25px] text-[16px] rounded-md">
                                        Edit
                                    </a>
                                    <form onsubmit="return confirmSubmit(this, 'Hapus Data?')" action="{{ route('inputs.bbm_receipts.destroy', ['id' => $item->id]) }}" method="POST">
                                        @csrf
                                        @method('delete')
