    ...
    <div class="max-h-screen overflow-hidden" :class="sidebar?'w-10/12' : 'w-full'">
        ...
        <div class="w-full py-20 px-8 max-h-screen hide-scrollbar overflow-y-auto">
            ...
            <div class="bg-white rounded-lg p-6 h-full">
                <form x-data="{ submitForm: function() { document.getElementById('filterForm').submit(); } }" x-on:change="submitForm()" action="{{ route('inputs.tug-9.index-heavy') }}" method="GET" id="filterForm">
                    <div class="lg:flex items-center gap-5 w-full mb-3">
                        ...
                        <select id="heavy" name="heavy" class="w-[350px] h-[44px] rounded-md border px-2" autofocus>
                            <option selected disabled>Untuk Alat Besar</option>
                            @foreach ($heavies as $heavy)
                                <option value="{{$heavy->uuid}}" {{request('heavy') == $heavy->uuid ? 'selected' :''}}>{{$heavy->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr>
                                ...
                                <th class="border  bg-[#F5F6FA] h-[52px] text-[#8A92A6]">Alat Berat</th>
                                <th class="border  bg-[#F5F6FA] h-[52px] text-[#8A92A6]">#</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-[16px] font-normal border px-2 text-center">{{ $tug->heavyEquipment->name ?? '' }}</td>
                                <td class="text-[16px] font-normal border px-2 flex items-center justify-center gap-2">
                                    <a href="{{route('inputs.tug-9.detail-heavy',['id'=>$tug->id])}}" class="bg-[#1AA053] text-center text-white w-[80px] h-[25px] text-[16px] rounded-md">
                                        Detail
                                    </a>
                                </td>
                            </tr>
