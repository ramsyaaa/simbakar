            ...
            <div class="flex items-end justify-between mb-2">
                <div>
                    <div class="text-[#135F9C] text-[40px] font-bold">
                        Ubah BA Dibeli Perusahaan Lain
                    </div>
                    <div class="mb-4 text-[16px] text-[#6C757D] font-normal no-select">
                        <a href="{{ route('administration.dashboard') }}">Home</a> / <a href="{{ route('coals.usages.adjusment-outcomes.index') }}" class="cursor-pointer">BA Dibeli Perusahaan Lain</a> / <span class="text-[#2E46BA] cursor-pointer">Create</span>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-lg p-6">
                <form onsubmit="return confirmSubmit(this, 'Ubah BA Dibeli Perusahaan Lain ?')" action="{{ route('coals.usages.adjusment-outcomes.update',['id' => $adjusment->id]) }}" method="POST">
                    @csrf
                    @method('PATCH')
                        <div class="unloadings">
                            <div class="p-4 bg-white rounded-lg w-full">
                                ...
                                <div class="w-full">
                                    ...
                                    <div class="relative">
                                        <input required type="text" value="{{$adjusment->note}}" name="note" class="w-full lg:w-1/2 border rounded-md mt-3 mb-5 h-[40px] px-3">
                                        @error('note')
                                        <div class="absolute -bottom-1 left-1 text-red-500">
                                            {{ $message }}
                                        </div>
                                ...
                                <div class="w-full">
                                    ...
                                    <div class="relative">
                                        <input required type="date" name="tug_in_date"  value="{{$adjusment->tug_in_date}}" class="w-full lg:w-1/2 border rounded-md mt-3 mb-5 h-[40px] px-3"><br>
